...
class ChartPrefs {
  ...
  async load() {
    ...
    try {
      ...
      if (r === "auto" || r === "library" || r === "printed")
        this.referenceSource = r;
      ...
      if (gd === "off" || gd === "standard" || gd === "detailed")
        this.gridDensity = gd;
      if (
        df === "iso" ||
        df === "short" ||
      ...
      if (typeof bo === "number" && bo >= 0 && bo <= 200)
        this.bandOpacityPct = bo;
      if (an === "off" || an === "fast" || an === "normal" || an === "slow")
        this.animation = an;
      if (tm === "axis" || tm === "item") this.tooltipMode = tm;
      if (typeof sti === "boolean") this.showTitle = sti;
      if (xlm === "auto" || xlm === "rotate" || xlm === "hide")
        this.xLabelMode = xlm;
      if (
        tw === "all" ||
        tw === "30d" ||
  ...
  setX(v: boolean) {
    this.showXSlider = v;
    setDebounced(KEY_X_SLIDER, v);
  }
  setY(v: boolean) {
    this.showYSlider = v;
    setDebounced(KEY_Y_SLIDER, v);
  }
  setScrollZoom(v: boolean) {
    this.scrollZoom = v;
    setDebounced(KEY_SCROLL_ZOOM, v);
  }
  setReferenceSource(v: ReferenceSource) {
    this.referenceSource = v;
    setDebounced(KEY_REF_SOURCE, v);
  }

  setShowValues(v: boolean) {
    this.showValues = v;
    setDebounced(KEY_SHOW_VALUES, v);
  }
  setSmooth(v: boolean) {
    this.smooth = v;
    setDebounced(KEY_SMOOTH, v);
  }
  setScale(v: Scale) {
    this.scale = v;
    setDebounced(KEY_SCALE, v);
  }
  setShowBands(v: boolean) {
    this.showBands = v;
    setDebounced(KEY_SHOW_BANDS, v);
  }
  setUseNicknames(v: boolean) {
    this.useNicknames = v;
    setDebounced(KEY_USE_NICKNAMES, v);
  }
  setShowSymbols(v: boolean) {
    this.showSymbols = v;
    setDebounced(KEY_SHOW_SYMBOLS, v);
  }

  setLineWidth(v: number) {
    this.lineWidth = v;
    setDebounced(KEY_LINE_WIDTH, v);
  }
  setSymbolSize(v: number) {
    this.symbolSize = v;
    setDebounced(KEY_SYMBOL_SIZE, v);
  }
  setGridDensity(v: GridDensity) {
    this.gridDensity = v;
    setDebounced(KEY_GRID_DENSITY, v);
  }
  setDateFormat(v: DateFormat) {
    this.dateFormat = v;
    setDebounced(KEY_DATE_FORMAT, v);
  }

  setXAxisPadPct(v: number) {
    this.xAxisPadPct = v;
    setDebounced(KEY_X_AXIS_PAD_PCT, v);
  }
  setYAxisPadPct(v: number) {
    this.yAxisPadPct = v;
    setDebounced(KEY_Y_AXIS_PAD_PCT, v);
  }
  setYVisiblePadPct(v: number) {
    this.yVisiblePadPct = v;
    setDebounced(KEY_Y_VIS_PAD_PCT, v);
  }

  setShowTodayLine(v: boolean) {
    this.showTodayLine = v;
    setDebounced(KEY_SHOW_TODAY_LINE, v);
  }
  setShowTrendLine(v: boolean) {
    this.showTrendLine = v;
    setDebounced(KEY_SHOW_TREND_LINE, v);
  }
  setShowMeanLine(v: boolean) {
    this.showMeanLine = v;
    setDebounced(KEY_SHOW_MEAN_LINE, v);
  }
  setShowMinMaxMarkers(v: boolean) {
    this.showMinMaxMarkers = v;
    setDebounced(KEY_SHOW_MIN_MAX, v);
  }

  setSeriesType(v: SeriesType) {
    this.seriesType = v;
    setDebounced(KEY_SERIES_TYPE, v);
  }
  setStep(v: StepKind) {
    this.step = v;
    setDebounced(KEY_STEP, v);
  }
  setColorByFlag(v: boolean) {
    this.colorByFlag = v;
    setDebounced(KEY_COLOR_BY_FLAG, v);
  }
  setConnectNulls(v: boolean) {
    this.connectNulls = v;
    setDebounced(KEY_CONNECT_NULLS, v);
  }
  setBandOpacityPct(v: number) {
    this.bandOpacityPct = v;
    setDebounced(KEY_BAND_OPACITY, v);
  }
  setAnimation(v: AnimationSpeed) {
    this.animation = v;
    setDebounced(KEY_ANIMATION, v);
  }
  setTooltipMode(v: TooltipMode) {
    this.tooltipMode = v;
    setDebounced(KEY_TOOLTIP_MODE, v);
  }
  setShowTitle(v: boolean) {
    this.showTitle = v;
    setDebounced(KEY_SHOW_TITLE, v);
  }
  setXLabelMode(v: XLabelMode) {
    this.xLabelMode = v;
    setDebounced(KEY_X_LABEL_MODE, v);
  }
  setTimeWindow(v: TimeWindow) {
    this.timeWindow = v;
    setDebounced(KEY_TIME_WINDOW, v);
  }
 ...
 * structurally identical to `ChartPrefs` so `time-series.svelte` can take
