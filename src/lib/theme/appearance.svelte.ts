...
export type AccentName = 'violet' | 'blue' | 'emerald' | 'rose' | 'amber' | 'teal';
export type Density    = 'comfortable' | 'compact';
export type FontScale  = 'sm' | 'md' | 'lg';
export type FontFamily = 'sans' | 'serif' | 'mono';

export const ACCENT_PRESETS: { id: AccentName; label: string; swatch: string }[] = [
...
class AppearanceStore {
  ...
  apply() {
    ...
    root.setAttribute('data-reduce-motion', this.reduceMotion ? 'true' : 'false');
    // Fire a synthetic theme-change so chart themes pick up new
    // accent / chart-1 colors immediately.
    root.dispatchEvent(new CustomEvent('theme-change', { detail: 'appearance' }));
  }
  ...
  setAccent(v: AccentName)         { this.accent = v;       this.apply(); setDebounced(KEYS.accent, v); }
  setDensity(v: Density)           { this.density = v;      this.apply(); setDebounced(KEYS.density, v); }
  setFontScale(v: FontScale)       { this.fontScale = v;    this.apply(); setDebounced(KEYS.fontScale, v); }
  setReduceMotion(v: boolean)      { this.reduceMotion = v; this.apply(); setDebounced(KEYS.reduceMotion, v); }
  setFontFamily(v: FontFamily)     { this.fontFamily = v;   this.apply(); setDebounced(KEYS.fontFamily, v); }
