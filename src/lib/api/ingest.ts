...
export async function ingestPdf(path: string): Promise<IngestResult> {
  return invoke<IngestResult>('ingest_pdf', { path }, { timeoutMs: INGEST_TIMEOUT_MS });
}

export async function ingestPdfs(paths: string[]): Promise<IngestOutcome[]> {
  return invoke<IngestOutcome[]>('ingest_pdfs', { paths }, {
...
export async function subscribeProgress(
  ...
  unlisteners.push(await listen<IngestProgress>('ingest:progress', (e) => onFile(e.payload)));
  if (onBatch) {
    unlisteners.push(await listen<BatchProgress>('ingest:batch', (e) => onBatch(e.payload)));
  }
