  ...
  {:else if overviews.length > 0}
    ...
    {#if displayedSections.length === 0}
      <div class="card p-5 text-sm text-fg2">
        <div class="flex items-center gap-2 font-medium text-fg1"><Icon name="dashboard" size={16} /> Dashboard sections are hidden</div>
      ...
      {#each displayedSections as section (section)}
        {#if section === 'kpis'}
          <section class="grid grid-cols-2 md:grid-cols-4 gap-3" aria-label="Summary cards">
            ...
            <div class="card p-3"><div class="text-xs text-fg2">Critical results</div><div class="text-2xl font-semibold tabular-nums {kpis.recentCritical > 0 ? 'text-crit' : ''}">{kpis.totalCritical}</div><div class="text-[10px] text-fg3 mt-0.5">{kpis.recentCritical} recent</div></div>
          </section>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                {#each spotlight as p}
                  <a class="card p-3 border-l-4 {p.recent_critical_row_count > 0 ? 'border-crit' : 'border-warn'} hover:bg-bg3 transition-colors" href={`/patient/${p.id}`}>
                    ...
                    <div class="flex items-center gap-3 mt-1.5 text-xs">{#if p.recent_critical_row_count > 0}<span class="text-crit font-medium tabular-nums">{p.recent_critical_row_count} critical</span>{/if}{#if p.recent_abnormal_row_count > 0}<span class="text-warn font-medium tabular-nums">{p.recent_abnormal_row_count} abnormal</span>{/if}</div>
                    <div class="text-[11px] text-fg3 mt-1">{p.recent_report_count} recent report{p.recent_report_count === 1 ? '' : 's'}{#if p.latest_collection_date_iso} · {formatDate(p.latest_collection_date_iso)}{/if}</div>
                  </a>
                {/each}
              </div>
            </section>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {#each patientsByActivity as p}
                <a class="card p-3 hover:bg-bg3 transition-colors" href={`/patient/${p.id}`}>
                  ...
                  <div class="text-xs text-fg2 mt-1">{p.report_count} report{p.report_count === 1 ? '' : 's'} · {p.distinct_analyte_count} analytes</div>
                  <div class="text-[11px] text-fg3 mt-0.5">{#if p.latest_collection_date_iso}latest {formatDate(p.latest_collection_date_iso)}{#if p.earliest_collection_date_iso && p.earliest_collection_date_iso !== p.latest_collection_date_iso} · span {formatRelativeSpan(p.earliest_collection_date_iso, p.latest_collection_date_iso)}{/if}{:else}no reports yet{/if}</div>
                  {#if p.abnormal_row_count > 0 || p.critical_row_count > 0}<div class="flex items-center gap-2 mt-1 text-[11px]">{#if p.critical_row_count > 0}<span class="text-crit tabular-nums">{p.critical_row_count} crit</span>{/if}{#if p.abnormal_row_count > 0}<span class="text-warn tabular-nums">{p.abnormal_row_count} abn</span>{/if}</div>{/if}
                </a>
              {/each}
            </div>
          </section>
